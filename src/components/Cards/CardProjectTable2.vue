  ...
  <a-card :bordered="false" class="header-solid h-full" :bodyStyle="{padding: 0,}">
    <template #title>
      <a-row type="flex" align="middle">
        <a-col :span="24" :md="12">
          <h5 class="font-semibold m-0">{{ header }}</h5>
        </a-col>
        <a-col :span="24" :md="12" style="display: flex; align-items: center; justify-content: flex-end">
          <a-radio-group v-model="projectHeaderBtns" v-for="radio in radios" size="small">
            <a-radio-button value="radio.status">{{radio.title}}</a-radio-button>

          </a-radio-group>
          <a-radio-group v-model="projectHeaderBtns" size="small">
            <a-radio-button value="add">
              <router-link :to="link">QO'SHISH</router-link>
            </a-radio-button>
          </a-radio-group>
        </a-col>
      </a-row>
    ...
    <a-table :columns="columns" :data-source="data" :pagination="false">
      ...
      <template slot="completion" slot-scope="completion">
        <div class="progress-right">
          ...
          <a-progress class="m-0" :percent="completion.value || completion.value == 0 ? completion.value : completion"
